...
| **Ruler** | `⌃⌥R` and the pointer sizes whatever it is over: Plonk reads the pixels and finds where the button, the cell or the gap stops, so nothing has to be aimed at. Drag for a straight-line distance instead. Points and pixels both, because a 44-point tap target is 88 pixels and only one of those numbers is in the asset. Click copies the number, Escape ends it |
